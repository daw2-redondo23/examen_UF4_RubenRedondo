...
class RealtimeClient {
  ...
  push(data) {
    ...
    if (this.isConnected()) {
      if (["broadcast", "presence", "postgres_changes"].includes(event2)) {
  ...
  _onConnMessage(rawMessage) {
    this.decode(rawMessage.data, (msg) => {
      let { topic, event: event2, payload, ref } = msg;
      if (ref && ref === this.pendingHeartbeatRef || event2 === (payload === null || payload === void 0 ? void 0 : payload.type)) {
        this.pendingHeartbeatRef = null;
      }
...
class GoTrueClient {
  ...
  constructor(options) {
    ...
    if (isBrowser() && globalThis.BroadcastChannel && this.persistSession && this.storageKey) {
      ...
      (_a = this.broadcastChannel) === null || _a === void 0 ? void 0 : _a.addEventListener("message", (event2) => {
        this._notifyAllSubscribers(event2.data.event, event2.data.session, false);
      });
    }
    this.initialize();
  }
  ...
  _notifyAllSubscribers(event2, session, broadcast = true) {
    if (this.broadcastChannel && broadcast) {
      this.broadcastChannel.postMessage({ event: event2, session });
    }
    this.stateChangeEmitters.forEach((x) => x.callback(event2, session));
  }
  /**
...
class SupabaseClient {
  ...
  _listenForAuthEvents() {
    let data = this.auth.onAuthStateChange((event2, session) => {
      this._handleTokenChanged(event2, session === null || session === void 0 ? void 0 : session.access_token, "CLIENT");
    });
    return data;
  }
  _handleTokenChanged(event2, token, source) {
...
class Tiquet {
  ...
  static async getAll() {
    let { data: tiquets, error } = await supabase.from("tiquets").select("*");
    if (error) {
      throw new Error(error.message);
    }
  ...
  static async getById(id) {
    let { data: tiquets, error } = await supabase.from("tiquets").select("*").eq("id", id).single();
    if (error) {
      throw new Error(error.message);
    }
  ...
  async update() {
    ...
    const { error } = await supabase.from("tiquets").update({
      estado: "resuelto",
...
const panel = {
  ...
  script: async () => {
    ...
    let tablaResueltos = `<thead>
                            ...
                            <tbody>`;
    let tiquets = await Tiquet.getAll();
    console.log(tiquets);
    tiquets.forEach((tiquets2) => {
      if (tiquets2.estado == "pendiente") {
        tablaPendientes += `<tr data-id="${tiquets2.id}">
                                        <td>${tiquets2.codigo}</td>
                                        <td>${tiquets2.created_at}</td>
                                        <td>${tiquets2.aula}</td>
                                        <td>${tiquets2.grupo}</td>
                                        <td>${tiquets2.ordenador}</td>
                                        <td>${tiquets2.descripcion}</td>
                                        <td>${tiquets2.alumno}</td>
                                        <td><button data-id="${tiquets2.id}" class="btn btn-success btnResolver" title="Resolver ticket">Resolver</button></td>
                                        ...
                                        <td><button data-id="${tiquets2.id}" class="btn btn-danger btnEliminar" title="Eliminar ticket"><i class="bi bi-trash3"></i>
                                        </i>
                                        </button></td>
                                    </tr>`;
      } else {
        tablaResueltos += `<tr data-id="${tiquets2.id}">
                                    <td>${tiquets2.codigo}</td>
                                    <td>${tiquets2.created_at}</td>
                                    <td>${tiquets2.fecha_resuelto}</td>
                                    <td>${tiquets2.aula}</td>
                                    <td>${tiquets2.grupo}</td>
                                    <td>${tiquets2.ordenador}</td>
                                    <td>${tiquets2.descripcion}</td>
    ...
    document.querySelector("#ticketsPendientes").innerHTML = tablaPendientes;
    tablaResueltos += `</tbody>`;
    document.querySelector("#ticketsResueltos").innerHTML = tablaResueltos;
    let btnResolver = document.querySelectorAll(".btnResolver");
    btnResolver.forEach(async (boton) => {
      boton.addEventListener("click", async (e) => {
        e.preventDefault();
        let id = event.target.dataset.id;
        let tiquetAresolver = await Tiquet.getById(id);
        tiquetAresolver.update();
        location.reload();
      });
    });
